...
const GameBoard = ({
  ...
  return (
    <div className="relative p-4">
      <div className="flex flex-col-reverse lg:flex-row lg:items-start lg:justify-between gap-4 relative">
        {/* Game Info */}
        <div className="bg-gray-700 p-6 rounded-lg shadow-inner border border-gray-600 w-full lg:w-2/3">
          ...
          {currentGame.status === 'playing' && (
            <p className="text-center text-xl font-bold text-yellow-300 mb-2">
              {isMyTurn
                ? "It's your turn!"
                : `It's ${currentGame.players.find(p => p.id === currentGame.currentTurnPlayerId)?.name}'s turn.`}
            </p>
          )}
        </div>
        {/* Floating Scores */}
        <div className="bg-gray-800 bg-opacity-90 p-4 rounded-lg shadow-lg border border-gray-600 w-full lg:w-1/3 z-50">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
        {otherPlayers.map((player) => {
          const revealedHand = currentGame.revealedHands?.find(rh => rh.id === player.id)?.hand || [];

          return (
            <div
              key={player.id}
              className="bg-gray-700 p-4 rounded-lg shadow-inner border border-gray-600 text-center"
            >
              <h3 className="text-indigo-300 font-semibold text-lg mb-2">{player.name}</h3>

              {/* Render cards based on game status */}
              <div className="flex flex-wrap justify-center items-center gap-1">
                {currentGame.status === 'show_called'
                  ...
                  : player.hand.map((_, index) => (
                    <Card key={index} isBack={true} disabled={true} />
                  ))}
              </div>

              <p className="text-sm text-gray-400 mt-1">
                {player.id === currentGame.currentTurnPlayerId ? 'Playing...' : 'Waiting'}
              </p>
            </div>
          );
        })}
      ...
      {/* Piles */}
      <div className="bg-gray-700 p-4 rounded-lg shadow-inner border border-gray-600 text-center flex flex-col md:flex-row justify-center items-center gap-8 mb-4">
        <h3 className="text-xl font-semibold mb-2 text-indigo-300">Table</h3>
        {/* Draw Pile */}
        <div className="flex flex-col items-center">
          <Card isBack={true} disabled={true} />
        </div>
        ...
        <div className="flex flex-col items-center">
          {topCard ? (
            <div className="flex justify-center items-center gap-2">
              ...
              <Card value={topCard.rank} suit={topCard.suit} disabled={true} />
            </div>
          ) : (
